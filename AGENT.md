...
- 当前实施分支为 `codex/v0.2.3-overlay-controls`，HEAD 仍为与 `origin/main` 同步的 `8ba07ade4e0b5437534a8d178f4a7bea81b02542`，尚未设置上游；该提交是 ready [PR #12](https://github.com/realMisakaMikoto/junrei_navi/pull/12) 经 Chrome rebase 合并后的 v0.2.3 实现 tip。当前工作树保留上一阶段未提交的 `AGENT.md` / `docs/releases/v0.2.3.md` 验收记录，并叠加悬浮控制改造；仍未提交、推送或创建新 PR。最终 PR CI run `30733492538` 与 main CI run `30733984142` 的 backend、verify、API 26、API 37 均全部成功。原实施分支 `codex/v0.2.3-japan-transit` 及同名上游保留；v0.2.2 功能提交 `0423bd06e67694e28f15c378a948a9bcd9d4ff28` 经 [PR #11](https://github.com/realMisakaMikoto/junrei_navi/pull/11) rebase 合并，发布源码提交为 `308d4c2afcc9681335e9b60d68cbb2891b025442`。
...
- 当前源码最新自动证据：44 个 JVM 套件 / 243 个测试，0 失败、0 错误、0 跳过；Debug/Release Lint 0 问题，Debug/Release/AndroidTest Kotlin 编译、Debug/AndroidTest APK、Release R8、Navigation 反射、源码凭据、APK 内容和 Natural Earth 入包哈希审计均通过。后端类型检查、构建、29/29 测试及生产依赖审计仍适用。PR #12 与 main 的既有功能门禁全部成功；当前分支尚未创建新 PR，不能沿用旧 CI 冒充新分支远端证据。
- 诚实边界：用户已授权并完成旧调试包清理、精确公开 v0.2.2 基线安装、Room 2 可见行程建立、正式 v0.2.3 原位覆盖，以及核心地区分流和新版悬浮控制真机验收；用户确认核心功能无问题。ADB mock 已真实推进到 `ARRIVING`，但不冒充 GNSS，也未点击确认到达；本次“提前离开”改动尚未安装个人真机，证据为 JVM、编译、Lint、静态审计及后续 CI。未运行 `connectedAndroidTest`，未卸载、清理或覆盖测试包；稳定发布仍须本分支 PR/main CI 和精确公开资产门禁。
- 正式签名环境继续使用工作区外原 keystore 和两份 Windows 当前用户范围 DPAPI 文件，未重新生成签名。最新候选在单个无守护 Gradle 子进程中临时注入四项 `ANITABI_*`，`finally` 清除环境变量；243 个 JVM、双 Lint 与 `assembleRelease` 明确 `BUILD SUCCESSFUL`。Release APK 为 `cn.anitabi.navigator` / `0.2.3 (9)`、minSdk 26、targetSdk 37，49,227,286 字节，SHA-256 `67f2bd35e2aca560a57a3126bed9eb76cefb667fec2872b3e0771a5e24ed9a79`；`apksigner` 验证为 v2、单一 RSA-4096 签名者，固定证书与 v0.2.2 完全一致。该本地产物尚不是公开 Release 资产。
...
- v0.2.3 当前源码已有报告：243 个 JVM 测试 / 44 个套件，0 失败、0 错误、0 跳过；Debug/Release Lint 0 问题；Debug/Release/AndroidTest Kotlin、Debug/AndroidTest APK、Release R8、Navigation 反射、源码凭据和 APK 内容审计通过；后端 29/29、类型检查、构建及生产依赖审计通过。既有实现 PR `30733492538` 和 main `30733984142` 的 backend、verify、API 26、API 37 均成功；当前分支仍待新 PR 和新 CI。
- v0.2.3 最新本地正式签名 Release APK 为 49,227,286 字节、SHA-256 `67f2bd35e2aca560a57a3126bed9eb76cefb667fec2872b3e0771a5e24ed9a79`；包名/版本、v2、单一 RSA-4096 签名者和固定证书均已独立核对。核心地区分流、Google Maps 交接、升级保留数据、悬浮控制及 ADB mock `ARRIVING` 有此前候选的真机证据；新增提前离开仅有本地自动证据，最新 APK 未安装个人真机。公开 Release 仍不存在，不得把本地候选或旧 CI 冒充公开资产和新分支远端结果。
...
- v0.2.3 已通过 Chrome 将 PR #12 rebase 合并到 `main@8ba07ade`；本地 231 个 JVM、双 Lint、Debug/AndroidTest、Release R8、后端 29/29，以及最终 PR/main 两轮 backend/verify/API 26/API 37 均已通过。四轮先前 PR CI 暴露的问题均已在保持严格并发与“不复活更旧行程”边界下完成修复。合并后正式签名 APK 已使用固定 RSA-4096 证书重建并通过三项审计；随后正式签名真机升级、地区分流及新版悬浮控制的实体门禁均完成。稳定 v0.2.3 只剩悬浮控制分支的 PR/main CI、标签、发布工作流和精确公开资产兼容门禁，完成前不得把本地候选当作公开 Release。
...
- 2026-08-02：按用户要求在日本外部公交 `DWELLING` 增加“提前离开”，悬浮面板与“停留中”并排，应用内和通知同步；点击经既有 `MODE_NEXT`/CAS 一次结束停留并启动下一段，末站完成，自然到期仍保留 `NEXT_STOP`。复核最新 Maps URL 与后台 Activity 官方规则；243 个 JVM、双 Lint、Debug/AndroidTest/Release Kotlin、正式签名 R8 和三项审计通过。最新候选 49,227,286 字节、SHA-256 `67f2bd35e2aca560a57a3126bed9eb76cefb667fec2872b3e0771a5e24ed9a79`，固定签名不变；首次因使用错误签名变量名在配置阶段停止，改用 Gradle 声明的准确名称后闭环。提交前把两个审计串在 30 秒命令时限内导致工具超时；根因是 R8 反射审计实际需约 55 秒，拆开并把单项时限提高到 120 秒后，源码凭据审计与 R8 审计均通过。改动未安装个人真机，分支仍待提交、PR、CI 和发布。
