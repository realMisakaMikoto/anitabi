...
- 当前任务分支为 `codex/v0.2.3-release-evidence`，基线为 `main@774064fc51181c32ff639c32d356aa76e86b7ae2`。移除 UID 日额度与 UID 突发限制的 [PR #14](https://github.com/realMisakaMikoto/junrei_navi/pull/14) 已通过 Chrome rebase 合并；main CI run `30750623097` 的 backend、verify、API 26 和 API 37 均成功。本分支只修复 `backend/deploy/backup.sh` 的 Git 可执行位并记录真实部署/发布状态；原 v0.2.3 标签仍为 `f173ebafef87c246de1e7d86e5e52979fdd18660`，不得移动或声称能重现替换候选。
- 非草稿、非预发布的 [v0.2.3 Release](https://github.com/realMisakaMikoto/junrei_navi/releases/tag/v0.2.3) 仍为 Latest Release，但公开 APK 尚未执行本次同版本替换，仍是 49,227,274 字节、SHA-256 `47374031a2836949179a9dc3c611e05e071bcb9a527f314253ad4a0d3a3e4a33`。精确合并源码 `main@774064fc` 的新候选为 49,227,286 字节、SHA-256 `b5a125386a511151139f5ff69a4b88d7101af822b53d920db5f45a00aca75eec`；必须等新后端成功部署后才能替换公开资产和同步 Release 正文。完整证据与阻断见 `docs/releases/v0.2.3.md`。
...
- 当前公开稳定版为 `v0.2.3`：`anitabi-v0.2.3.apk` 共 49,227,274 字节，SHA-256 `47374031a2836949179a9dc3c611e05e071bcb9a527f314253ad4a0d3a3e4a33`。独立重下载确认 APK 为 `cn.anitabi.navigator` / `0.2.3 (9)`、minSdk 26、targetSdk 37、APK Signature Scheme v2、单一 RSA-4096 签名者，固定证书 SHA-256 为 `9679c83769368c7150f629d9cba3c0e5d633fa7f1043ce251fdba6c7c64fb00a`；APK 内容审计及 API 26/API 37 精确公开资产门禁均通过。公开校验资产为 85 字节，其 GitHub 资产摘要 SHA-256 为 `ed6794708ea17808e5de92ed7bef9ecab9cdb1097713d92f5ac729100a7415c2`。
...
- 最后记录的已部署后端运行树仍来自 `0902e484c9d6e8745febea665c0a2c293839cf14`。2026-08-02 新部署在 systemd 备份门禁失败后自动完整回滚；其后公共健康检查再次实时确认为 `service=ok`、`database=ok`。新配额源码尚未部署，不能把成功镜像构建或候选容器健康冒充生产切换完成。
...
- 报错闭环（本次生产部署）：候选 archive 的 Git blob 校验因跨主机换行语义失败，改为候选原始 SHA-256；生产工作树原始哈希又因 CRLF 假报漂移，改回生产 Git blob 校验。镜像随后从精确 `main@774064fc` 构建成功，但无 healthcheck 的无关容器没有 `.State.Health`，快照改为稳定 inspect 后可空提取 Health。下一次已完成数据库备份、新容器健康与安全属性验证，却在部署后 systemd 备份失败；根因是仓库 `backend/deploy/backup.sh` 为 `100644`，而旧生产文件依赖人工可执行位。部署器自动回滚旧源码、镜像和容器，loopback/公开健康均恢复，SQLite 未回滚；临时兼容用旧文件 `chmod --reference`，跟踪模式现修正为 `100755`。随后直接 scp/ssh 连接出现超时；单连接 Base64 管道又因远端解码无效在脚本前退出，改为原始 tar stdin。部署辅助 Python 的首轮 AST 检查因 PowerShell 外部管道在首字符加入 U+FEFF 而假报语法错误，直接按 `utf-8-sig` 读取实际文件后解析通过，文件字节本身无 BOM。Paramiko 与系统 `ssh.exe` 最终都在远端脚本前被密码认证拒绝。OpenSSH 一次性密码桥首版把随机 token 的二进制值与十六进制环境值比较，导致空密码；用一致十六进制 token 修正，并以非敏感值逐字节验证。监听线程关闭时的预期 socket 错误也已在 stop 状态正常收口。15 分钟无连接冷却后的单次系统 SSH 仍返回 `Permission denied (publickey,password)`，公共 API 同时实时健康，故当前阻断是服务器未接受既有 root 凭据而非应用、镜像或数据库故障。恢复条件是服务器重新接受现有凭据；本任务没有修改密码、SSH、端口、防火墙、主机密钥或登录策略，也没有替换公开 APK。PR #15 监控时一次 GitHub API TLS 握手超时，原查询立即重试后确认同一 run 仍在正常执行，未重跑或修改 CI。
...
- v0.2.3 同版本额度策略更新的当前证据：PR #14 已合并为 `main@774064fc`，main CI run `30750623097` 的 backend、verify、API 26、API 37 全部成功；后端类型检查、构建及 29/29 测试，Android 245/245、双 Lint、Debug/AndroidTest、正式签名 R8 与三项审计均通过。精确合并源码重新构建候选为 `cn.anitabi.navigator` / `0.2.3 (9)`、minSdk 26、targetSdk 37，49,227,286 字节、SHA-256 `b5a125386a511151139f5ff69a4b88d7101af822b53d920db5f45a00aca75eec`，签名为 v2、单一 RSA-4096 与固定证书。部署主机镜像构建成功但生产在后备份门禁失败后已完整回滚，之后 SSH 认证受阻；公开资产仍是旧哈希，不能冒充已替换。
...
- 稳定 v0.2.3 已发布：PR #12 完成日本公共交通主体实现，PR #13 完成可拖动/缩放/悬浮球控制与停留态“提前离开”，均通过 Chrome rebase 合并；最终 `main` 与标签为 `f173ebaf`。243 个 JVM、双 Lint、Debug/AndroidTest、Release R8、后端 29/29、PR/main 两轮 backend/verify/API 26/API 37、签名发布和精确公开资产 API 26/API 37 门禁全部成功。正式签名真机升级、地区分流、悬浮控制和 ADB mock `ARRIVING` 已验收；公开 APK 也已独立重下载核对固定 RSA-4096 证书与内容。“提前离开”未另行安装个人真机，不能把自动门禁冒充实体点击。
...
- 2026-08-02：v0.2.3 移除个人请求限制的 PR #14 已经 Chrome rebase 合并为 `main@774064fc`，main CI 四项全绿；精确合并源码正式候选 SHA-256 为 `b5a125386a511151139f5ff69a4b88d7101af822b53d920db5f45a00aca75eec`，固定签名不变。部署主机实际镜像构建通过；一次切换因 archive 中 `backup.sh` 缺可执行位而在后备份门禁失败，自动回滚旧源码、镜像和健康容器成功，SQLite 未回滚，现将 Git mode 修正为 `100755`。其后 Paramiko 与系统 `ssh.exe` 均在脚本前被服务器密码认证拒绝，修正并验证空密码桥、冷却 15 分钟后仍相同；公共 API 保持健康。生产新后端和同版本公开 APK 替换尚未完成，Release 仍是旧资产，绝不越过先后端再 APK 的兼容顺序。
