...
- API 26 displayed the real Google package-installer permission activity, but the instrumentation helper allowed only four seconds for the dialog to leave focus. It now waits up to fifteen seconds, recognizes completion by the permission activity leaving focus, waits for the application to regain focus before using a single Back fallback, and waits for UI automation to become idle after the accessibility click. A fresh wiped local API 26 AVD repeated the complete real permission, service disclosure, search entry, and restart flow successfully in 10.389 seconds.
- API 37 restored the saved tour, stable offline notice, first completed point, and progress in its uploaded 320-by-640 UI hierarchy. The workflow had incorrectly required the second point name even though that row was below the small runner viewport. Recovery assertions now require the visible saved first point and its completed state, alongside the existing tour and offline-message checks, on both launches. All four revised markers were verified against the failing run's UTF-8 evidence artifact.
- Recompiled both Debug and AndroidTest APKs successfully after these test-only changes, and `git diff --check` reports no whitespace errors. No production application behavior, secret, VPS state, SSH setting, phone data, or release artifact changed during this correction.
