...
- 当前实施分支为 `codex/v0.2.3-japan-transit`，已跟踪同名上游。v0.2.3 主实现提交为 `ff75d8b63ce8ffc7bc97ca84c7733cc2d47cf814`，首轮冷恢复修复提交为 `ae835fb040d4b9747510c30f0a66011b22df6fba`，均已推送到 ready [PR #12](https://github.com/realMisakaMikoto/junrei_navi/pull/12)；第二轮模拟器故障修复也已进入当前分支，等待 PR 新 CI 确认，PR 尚未合并。任务起点 `HEAD` 为 `8d999c5e4c102b2a43e663653ea18306af1b193c`，当时与本地 `main` 及其上游完全一致（ahead/behind 0/0）。v0.2.2 功能提交 `0423bd06e67694e28f15c378a948a9bcd9d4ff28` 经 [PR #11](https://github.com/realMisakaMikoto/junrei_navi/pull/11) rebase 合并；发布源码提交为 `308d4c2afcc9681335e9b60d68cbb2891b025442`。
...
- 当前源码最新本地自动证据：第二轮 CI 修复后再次全量执行，43 个 JVM 套件 / 228 个测试，0 失败、0 错误、0 跳过；Debug/Release Lint、Debug/Release/AndroidTest Kotlin 编译均通过。主实现阶段另已完成 Debug/AndroidTest APK、后端类型检查/构建/29 个测试/生产依赖审计、Release R8、Navigation 反射、源码凭据及 Natural Earth 入包哈希审计；当时的 Debug APK 为 `cn.anitabi.navigator` / `0.2.3 (9)`，78,581,900 字节，SHA-256 `702c2b8314a9792b8936ef70bed895ac59936a79287ec8f3e35fb4256b2518bc`，AndroidTest APK 为 1,170,750 字节，SHA-256 `44c38b01b6d84edd7204797955b74bc2465518da5564baed0d22e6068642d71b`。这两个哈希早于后续 PR 修复，不得冒充当前最终发布候选。
...
- v0.2.3 当前工作树已有报告：228 个 JVM 测试 / 43 个套件，0 失败、0 错误、0 跳过；Debug/Release Lint 0 问题；Debug/Release/AndroidTest Kotlin 编译通过。主实现阶段的 Debug/AndroidTest APK、Release R8、Navigation 反射、后端 29 个测试、类型检查、构建和生产依赖审计均通过，后端生产依赖漏洞为 0；后续 PR 恢复修复尚待远端模拟器复验。
- v0.2.3 主实现阶段已有本地正式签名 Release APK：49,210,374 字节、SHA-256 `c48fdbbcf4ff90e4c97ea178ed0c7888297a895048b6047b6d791211083b9201`，包名/版本、v2、单一 RSA-4096 签名者和固定证书均已独立核对；该 APK 早于后续 PR 恢复修复，不能作为当前最终发布资产。ready PR #12 已创建，前两轮 PR CI 的 backend/verify 均通过，但模拟器先后暴露冷恢复 Room CAS、活动指针和 warm-cache 规范化问题；当前修复已完成本地验证并等待新 CI。尚无公开发布或 v0.2.3 真机证据。唯一连接设备是个人真机，本任务未获安装/运行授权，所以本地 instrumentation 仅编译未执行。不得把本地候选、失败 CI 或旧 v0.2.2 证据冒充 v0.2.3 公开资产/真机结果。
...
- v0.2.3 已在 PR #12 提交日本公交纯本地分流、用户主动的外部 Google 地图分段执行、独立状态机/控制入口、恢复和未来点编辑；本地 JVM/Lint/Debug/AndroidTest/R8/后端门禁及同一固定证书的主实现签名候选均已完成。前两轮 PR CI 依次暴露冷恢复 Room 起点规范化、失败后活动指针残留和 warm-cache 规范化回归，均已按严格非对称等价边界完成本地修复并扩展到 228 个 JVM 测试；等待 API 26/API 37 新 CI，PR 尚未合并，当前源码的最终签名资产、真机与公开发布仍未完成。
