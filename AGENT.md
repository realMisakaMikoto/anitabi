...
## 2026-08-01 - Task 62: Google Navigation cancelled-task crash repair and physical-device verification

### Diagnosis and implementation

- Re-read the complete current 1,571-line `AGENT.md` in UTF-8 before starting this task. Investigated the reported probabilistic `IllegalStateException: Task was cancelled.` crash against the current Release mapping, local Navigation SDK 7.8.0 bytecode, application lifecycle code, and Google's current official Navigation SDK reference.
- The mapped stack frames are inside Navigation SDK 7.8.0. The application had registered the SDK `ListenableResultFuture` result listener and called `cancel(true)` when its coroutine was cancelled. The SDK delivers a cancelled listener failure on the UI executor by throwing `IllegalStateException`, which exactly matches the supplied stack. Google's reference also documents that `Navigator.cleanup()` cancels pending destination requests, so merely removing the application-side `cancel(true)` while retaining the fatal listener would not make cleanup safe. The evidence does not implicate the API key, route parameters, Firebase Task bridge, or ordinary network failure.
- Replaced the listener bridge for `Navigator.setDestinations()` with an interruptible blocking wait on `Dispatchers.IO`. The application no longer registers the SDK result listener and never calls `cancel()` on this Future. Application coroutine cancellation still propagates normally; an SDK-originated cancellation while the caller remains active becomes a recoverable navigation-route error; and failed Future results retain their original cause type and message.
- Added four focused JVM regressions covering successful completion without listener registration or Future cancellation, application coroutine cancellation without a late side effect, SDK-originated cancellation while the caller remains active, and unwrapping of the original failed-result cause. Independent read-only reviews found no actionable correctness issue in the final task diff.

### Local and signed-build verification

- The complete `testDebugUnitTest`, `lintDebug`, and `assembleDebug` run passed. The JVM report contains 125 tests in 26 suites with zero failures, errors, or skips. Debug Lint contains only the same four non-blocking Compose performance hints.
- Built the production-signed Release APK using the existing fixed external signing identity, with signing values confined to a child-process environment and cleared afterward. `testDebugUnitTest`, `lintRelease`, `assembleRelease`, the Navigation SDK R8 reflection audit, tracked-source credential audit, Release APK content audit, and `git diff --check` passed. Release Lint contains the same four hint-only findings.
- The ignored local `app-release.apk` is 49,058,377 bytes with SHA-256 `6be527745502ed5a865ea0f63d07766dd3dd89899c8c591400b1c72c5beacc8b`. Package inspection reports `cn.anitabi.navigator`, version code 7, version name 0.2.1, minimum SDK 26, and target SDK 37. Signature inspection reports APK Signature Scheme v2, one RSA-4096 signer, and certificate SHA-256 `9679c83769368c7150f629d9cba3c0e5d633fa7f1043ce251fdba6c7c64fb00a`, preserving update compatibility.

### Physical-device verification and state boundary

- With the user's explicit authorization, overlaid the signed local build on the Xiaomi 15T Pro with `adb install -r`; did not uninstall the application or clear its data. The original installation time, completed onboarding, permissions, and previously stored application data were retained. The installed APK hash exactly matched the new local build.
- Cold launch succeeded and the Google map rendered. Performed six bounded one-destination start attempts, including three rapid UI stop cycles after approximately 300, 800, and 1,500 milliseconds. The application process remained unchanged, the crash buffer gained no new application crash or `Task was cancelled` record, and a final normal navigation start displayed active Google guidance. Navigation was then stopped normally and the application was left on the route-preview screen.
- The test used the device's pre-existing mock-location state; no real-GNSS claim is made. To keep the test bounded, the application's selected plan was changed from the previous nine-point selection to two nearby test points and was not restored. No permission, mock-location, network, SSH, password, Google/Firebase project, VPS, DNS, firewall, or unrelated system setting was changed.
- The physical test made one 2-by-2 road matrix request, one road route request, and at most six Navigation destination reservations; there were no transit requests or automatic billing retries. One rejected attempt to stop the non-exported service from the shell made no application-state change; all effective stops were performed through the application UI.
