...
- [下载 v0.2.1 RC7](https://github.com/realMisakaMikoto/anitabi/releases/tag/v0.2.1-rc.7)：当前测试版，已由用户在真机确认 Google 底图与巡礼点、步行和驾车路线可正常使用；公交后来复测失败，尚未通过验收。
...
这是 RC7 没有翻译好的配额提示，不代表手机断网。道路导航启动前会为当前批次预留目的地；每个匿名用户每天最多预留 20 个。比如 18 段行程会一次使用 18 个，当天此前已经使用 3 个或更多时就会被拒绝。出现该提示时，Google 原生导航实际上没有开始，行程和进度仍保留在本机。

### 为什么公交显示“本区域暂无开放公交数据”？

RC7 的这句提示不准确，不能证明现实中或 Google 地图里没有公交。进一步核对生产响应和 [Google Maps Platform 官方 FAQ](https://developers.google.com/maps/faq#transit_directions_countries) 后已经确认：面向开发者的 Routes API 公交功能明确排除日本的 Google Transit 合作方；消费版 Google 地图使用的并不是同一套开放接口。因此，东京等日本地区即使能在 Google 地图中查询公交，RC7 仍无法在应用内取得对应路线，公交也未通过验收。后续源码不会再把全步行结果伪装成公交；在选定合法、可持续的日本公交数据方案前，不会宣称日本应用内公交可用。
