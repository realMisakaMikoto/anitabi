...
- [下载 v0.2.2](https://github.com/realMisakaMikoto/junrei_navi/releases/tag/v0.2.2)：当前稳定版，包含全新界面、地图显示修复和无线路详情。
...
从 v0.2.1 更新到 v0.2.2 时，请直接覆盖安装，不要先卸载应用或清除数据。应用会保留导览状态、作品与点位选择、行程顺序、设置和导航进度；本版本没有数据库迁移。
...
- 公共交通路线不支持中国大陆、日本和印度。原因是当前使用的 Google API 无法提供本应用在这些地区所需的公交数据，并非本项目主动屏蔽；本项目（巡礼手帳）短期内也不计划另外接入第三方公交数据源。消费版 Google 地图能显示当地公交，不代表开发者接口能够返回相同结果。
...
- 当你在无线路详情中主动选择“在 Google 地图中查看路线”时，应用会把该失败分段的精确起终点交给手机上的 Google 地图，并预选公共交通模式。
...
当前版本不支持中国大陆、日本和印度的公共交通路线。根因是当前 Google API 无法提供本应用在这些地区所需的公交数据，不是本项目主动屏蔽。Google 的[官方说明](https://developers.google.com/maps/faq#transit_directions_countries)明确列出的例外是日本的 Transit 合作方和印度铁路 IRCTC；结合本项目当前 Google API 路径的实际能力，中国大陆和印度整体也无法取得本应用所需的可用公交结果。消费版 Google 地图使用的数据范围并不等同于开发者 Routes API。本项目（巡礼手帳）短期内不计划另外接入第三方公交数据源；在这些地区请使用 Google 地图或当地交通应用查询公交。其他地区也只有在 Google Routes 实际返回公交路线时才能规划；应用不会把全步行结果伪装成公交。
...
从 v0.2.1 直接覆盖安装 v0.2.2 不需要数据库迁移。不要卸载、不要清除应用数据；已保存的行程和进度会继续保留。从 v0.2.0 更新时，旧路线内容仍会按既有隐私规则移除，并在联网后重新生成。
