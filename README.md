...
- 公共交通路线不支持中国大陆、日本和印度。原因是当前使用的 Google API 无法提供本应用在这些地区所需的公交数据，并非本项目主动屏蔽；本项目（巡礼手帖）短期内也不计划另外接入第三方公交数据源。消费版 Google 地图能显示当地公交，不代表开发者接口能够返回相同结果。
...
v0.2.1 不支持中国大陆、日本和印度的公共交通路线。根因是当前 Google API 无法提供本应用在这些地区所需的公交数据，不是本项目主动屏蔽。Google 的[官方说明](https://developers.google.com/maps/faq#transit_directions_countries)明确列出的例外是日本的 Transit 合作方和印度铁路 IRCTC；结合本项目当前 Google API 路径的实际能力，中国大陆和印度整体也无法取得本应用所需的可用公交结果。消费版 Google 地图使用的数据范围并不等同于开发者 Routes API。本项目（巡礼手帖）短期内不计划另外接入第三方公交数据源；在这些地区请使用 Google 地图或当地交通应用查询公交。其他地区也只有在 Google Routes 实际返回公交路线时才能规划；应用不会把全步行结果伪装成公交。
