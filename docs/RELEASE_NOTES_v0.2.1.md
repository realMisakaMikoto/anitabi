...
`v0.2.1-rc.N` 是正式签名预发布候选，不是稳定版。RC 已通过后端、生产 HTTPS、真实小额 Google 请求、并发硬额度、65 个 JVM 测试和 API 26/API 37 模拟器验证；在 Xiaomi 15T Pro 完成“不卸载、不清数据”的 v0.2.0→v0.2.1 覆盖安装、Google Navigation 条款/语音/偏航/锁屏和长行程换批前，不能把 RC 结果表述为稳定 v0.2.1 验收。
