...
`v0.2.1-rc.N` 是正式签名预发布候选，不是稳定版。RC2 增加了异常 Anitabi 可选文本字段兼容、Google 地图初始化崩溃兜底、选择状态恢复、道路局部重排、公交站点信息和步行/骑行安全提示。当前源码已通过后端、生产 HTTPS、真实小额 Google 请求、并发硬额度、69 个 JVM 测试和 API 26/API 37 模拟器既有门禁；RC2 的正式签名包仍须完成覆盖安装和真机复验，不能把候选结果表述为稳定 v0.2.1 验收。
