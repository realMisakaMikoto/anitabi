...
`v0.2.1-rc.N` 是正式签名预发布候选，不是稳定版。RC2 修复异常 Anitabi 可选字段与多作品联合选择；RC3 修复 Navigation SDK 与 R8 类合并造成的正式版崩溃。RC3 随后在另一台真机暴露 `IBitmapDescriptorFactory` 未初始化崩溃。RC4 删除客户端位图标记工厂依赖，并让点位地图操作失败时安全回退到列表；只有地图真实渲染和最小两点路线真机验收通过后，才能继续稳定版发布。
