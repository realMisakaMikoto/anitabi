...
`v0.2.1-rc.N` 是正式签名预发布候选，不是稳定版。RC2 已在真机确认异常 Anitabi 可选文本字段兼容与多作品联合选择，但其正式包地图仍会受到 Navigation SDK 7.8.0 与 R8 优化的反射访问冲突影响。RC3 按 SDK 消费者规则禁用不兼容的类合并，并固定反射注册器的包访问关系；它仍须通过 CI、正式签名覆盖安装和真机地图复验，不能把候选结果表述为稳定 v0.2.1 验收。
