...
class RoadNavigationBatchCoordinatorTest {
    @Test
    fun `production batches use the Navigation SDK destination ceiling`() {
        val coordinator = RoadNavigationBatchCoordinator(destinationCount = 61, initialLegIndex = 0)

        assertEquals(RoadNavigationAction.Load(0..24), coordinator.actionFor(0))
        coordinator.markLoaded(0..24)
        ...
        assertEquals(RoadNavigationAction.Load(25..49), coordinator.actionFor(25))
        coordinator.markLoaded(25..49)
        assertEquals(RoadNavigationAction.Load(50..60), coordinator.actionFor(50))
    }
